...
export default function App() {
  ...
  // Generate or retrieve persistent local visitor ID for server sessions
  ...
  // 1. Real-time visitor ping to Express / Google Cloud backend
  useEffect(() => {
    const vId = typeof visitorIdRef.current === 'function' ? (visitorIdRef.current as () => string)() : visitorIdRef.current;
    ...
    const pingServer = async () => {
      try {
        ...
        // Fallback offline mock values if server is booting
      }
    };
  ...
  // 2. Initialize MasterEngine and 3D Game Loop
  useEffect(() => {
  ...
  const handleUseItem = (item: InventoryItem) => {
    ...
    engine.setToast(`Used ${item.name}!`);
    setTick((t) => t + 1);
  };
